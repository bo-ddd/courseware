class Pagination {
    constructor(options) {
        ...
        this.pre = this.el.querySelector(options.pre);
        this.nextClass = options.next.substring(1);  // options.next   next
        this.next = this.el.querySelector(options.next);

        this.tabPageCallBack = options.tabPageCallBack;
        // 观察  数据劫持
        this.observer();
    ...
    init() {
        ...
        let handleClick = function () {
            ...
            // 非必填项
            if(_this.tabPageCallBack) _this.tabPageCallBack();
        }
